...
const CUSTOMER_QUERY_PARAM_CONFIG = [
  // <path>[<CUSTOMER_SORT_KEYS>]=<CUSTOMER_SORT_VALUES>
  // eg. path = ['sort', 'by', 'some'] ~ sort[by][some][firstName]=DESC ~ { sorting: { firstName: 'DESC' }
  { key: 'sorting', path: ['sort'], type: ENUM_KEYS_VALUES, keys: CUSTOMER_SORT_KEYS, values: CUSTOMER_SORT_VALUES },
  // <path>[<CUSTOMER_FILTER_KEYS>]=<any value>
  // eg. path = ['filter', 'by', 'some'] ~ filter[by][some][firstName]=anna ~ { filtering: { firstName: 'anna' } }
  { key: 'filtering', path: ['filter'], type: ENUM_KEYS, keys: CUSTOMER_FILTER_KEYS },
  // <path>=<INTEGER>
  // eg. path = ['page', 'number'] ~ page[number]=123 ~ { pageNumber: 123 }
  { key: 'pageNumber', path: ['page', 'number'], type: INT },
  // <path>=<INTEGER>
  // eg. path = ['page', 'size'] ~ page[size]=123 ~ { pageSize: 123 }
